...
- 2026-08-26 — A cache/config path is not "wired up" until you check what the library does with it. `with_cache_dir` stores the path verbatim while `Cache::default`/`from_env` append `hub`, so passing `MODEL_CACHE_DIR` through orphaned a populated 264MB cache in production. Read the dependency's path construction, not just its function name.
